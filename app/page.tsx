...
function Dashboard({ filter, setFilter, filteredRequests, setView }: { filter: string; setFilter: (v: string) => void; filteredRequests: typeof requests; setView: (v: View) => void }) {
  return <div className="content-grid">
    ...
    <SupplyFlowWidget />
    <ImpactSimulator />
    <section className="panel requests-panel"><div className="panel-head"><div><h2>Registros consolidados</h2><p>Estrutura oficial de inspeções e custos</p></div><span className="approval-progress">{inspectionData.length} registros</span></div><div className="table-wrap"><table><thead><tr><th>ID</th><th>Equipamento</th><th>Status</th><th>Data de inspeção</th><th>Responsável</th><th>Custo total</th></tr></thead><tbody>{inspectionData.map((item) => <tr key={item.id}><td><b>#{item.id}</b></td><td>{item.equipamento}</td><td><StatusBadge tone={item.status === 'Concluído' ? 'green' : 'amber'}>{item.status}</StatusBadge></td><td>{new Intl.DateTimeFormat('pt-BR').format(new Date(`${item.data_inspecao}T12:00:00`))}</td><td>{item.responsavel}</td><td><b>{currency.format(item.custo_total)}</b></td></tr>)}</tbody></table></div></section>

...
function ImpactSimulator() { const [reduction, setReduction] = useState(25); const overdueValue = 8600673.89; const released = overdueValue * reduction / 100; return <section className="panel impact-simulator"><div className="panel-head"><div><h2>Simulador de Redução de Atrasos</h2><p>Projete o valor liberado ao reduzir pedidos em atraso no FUP.</p></div><SlidersHorizontal /></div><div className="impact-body"><div className="impact-slider"><label>Redução estimada <b>{reduction}%</b></label><input type="range" min="0" max="100" value={reduction} onChange={(event) => setReduction(Number(event.target.value))} /><div><span>0%</span><span>100%</span></div></div><div className="impact-result"><small>Valor economizado / liberado</small><strong>{currency.format(released)}</strong><span>sobre R$ 8,60 mi em pedidos atrasados</span></div></div></section> }
...
function FupView() {
  const [query, setQuery] = useState(''); const [page, setPage] = useState(1); const [sortAsc, setSortAsc] = useState(false); const [toast, setToast] = useState(''); const [exportMode, setExportMode] = useState<'all' | 'overdue'>('all');
  const total = fupOrders.reduce((sum, item) => sum + item.value, 0)
  const filtered = fupOrders.filter((item) => `${item.material} ${item.description} ${item.po} ${item.supplier} ${item.status}`.toLowerCase().includes(query.toLowerCase())).sort((a, b) => sortAsc ? a.value - b.value : b.value - a.value); const visible = filtered.slice((page - 1) * 5, page * 5)
  function exportCsv() { const exportRows = exportMode === 'overdue' ? filtered.filter((item) => item.status === 'Atraso') : filtered; const csv = ['Material,Descrição,Pedido,Fornecedor,Prazo,Status,Valor', ...exportRows.map((item) => [item.material, item.description, item.po, item.supplier, item.date, item.status, item.value].join(','))].join('\\n'); const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); link.download = 'oms-fup.csv'; link.click(); setToast(`${exportRows.length} linhas exportadas em CSV!`); setTimeout(() => setToast(''), 2200) }
...
function OrdersView() {
  ...
  const [modal, setModal] = useState(false); const [page, setPage] = useState(1); const [toast, setToast] = useState(''); const [sortAsc, setSortAsc] = useState(false)
  const filteredRows = maintenanceOrders.filter((item) => `${item.number} ${item.equipment} ${item.sector} ${item.status}`.toLowerCase().includes(query.toLowerCase()) && (status === 'Todos' || item.status === status)).sort((a, b) => sortAsc ? a.value - b.value : b.value - a.value); const rows = filteredRows.slice((page - 1) * 5, page * 5)
  return <section className="panel full-panel orders-panel"><div className="panel-head"><div><h2>Ordens de Manutenção</h2><p>Planejamento e rastreabilidade das demandas da OMS.</p></div><button className="primary-button" onClick={() => setModal(true)}><Plus /> Criar Nova OM</button></div><div className="orders-toolbar"><TableTools query={query} setQuery={setQuery} page={page} setPage={setPage} total={filteredRows.length} /><div className="filter-tabs">{['Todos', 'PLAN', 'Aprovada'].map((item) => <button key={item} className={status === item ? 'selected' : ''} onClick={() => setStatus(item)}>{item}</button>)}</div></div><div className="table-wrap"><table><thead><tr><th>Número da OM</th><th>Equipamento</th><th>Setor</th><th>Status</th><th>Prioridade</th><th>Valor alocado</th><th /></tr></thead><tbody>{rows.map((item) => <tr key={item.number}><td><b>{item.number}</b><span>Aberta em 25/09/2026</span></td><td>{item.equipment}</td><td className="muted">{item.sector}</td><td><StatusBadge tone={item.status === 'Concluída' ? 'green' : item.status === 'PLAN' ? 'amber' : 'blue'}>{item.status}</StatusBadge></td><td>{item.priority}</td><td><b>{currency.format(item.value)}</b></td><td><button className="row-action" aria-label={`Abrir OM ${item.number}`}><ArrowUpRight /></button></td></tr>)}</tbody></table></div>{modal && <div className="modal-backdrop" role="presentation" onClick={() => setModal(false)}><div className="modal-card" role="dialog" aria-modal="true" aria-labelledby="new-om-title" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setModal(false)} aria-label="Fechar"><X /></button><p className="eyebrow">Cadastro rápido</p><h2 id="new-om-title">Criar Nova OM</h2><p className="modal-description">Vincule a demanda a um equipamento antes de solicitar materiais.</p><label>Número da OM<input placeholder="80014873076" /></label><label>Equipamento<input placeholder="Ex.: Motor hidráulico de içamento" /></label><label>Prioridade<select defaultValue="Alta"><option>Alta</option><option>Média</option><option>Baixa</option></select></label><button className="primary-button modal-submit" onClick={() => { setModal(false); setToast('OM cadastrada com sucesso!'); setTimeout(() => setToast(''), 2200) }}><Check /> Salvar OM</button></div></div>}{toast && <div className="toast success"><Check />{toast}</div>}</section>
}

function ChatView({ messages, draft, setDraft, sendMessage }: { messages: typeof initialMessages; draft: string; setDraft: (v: string) => void; sendMessage: () => void }) { const shortcuts = ['Consultar OM 80014873076', 'Verificar Saldo PL33', 'Solicitar Retirada no Balcão']; return <div className="chat-layout"><section className="chat-panel"><div className="chat-header"><div className="bot-avatar"><Bot /></div><div><h2>Assistente OMS</h2><p><span className="online-dot" /> Central de Abastecimento · online</p></div><button className="icon-button"><Settings /></button></div><div className="stepper"><div className="step active"><b>1</b><span>Identificação</span></div><div className="step"><b>2</b><span>Itens</span></div><div className="step"><b>3</b><span>Direcionamento</span></div><div className="step"><b>4</b><span>Confirmação</span></div></div><div className="messages">{messages.map((message, index) => <div className={`message-row ${message.role}`} key={index}>{message.role === 'assistant' && <div className="message-avatar"><Bot /></div>}<div className="message-bubble">{message.text.split('**').map((part, i) => i % 2 ? <strong key={i}>{part}</strong> : part)}</div><small>{index === messages.length - 1 ? 'agora' : '09:1' + index}</small></div>)}</div><div className="chat-shortcuts">{shortcuts.map((shortcut) => <button key={shortcut} onClick={() => setDraft(shortcut)}>{shortcut}</button>)}</div><div className="chat-composer"><textarea value={draft} onChange={(e) => setDraft(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing && e.keyCode !== 229) { e.preventDefault(); sendMessage() } }} placeholder="Descreva o material que você precisa..." aria-label="Mensagem para o Assistente OMS" /><button className="send-button" onClick={sendMessage} aria-label="Enviar mensagem"><Send /></button><div className="composer-hint">Enter para enviar · Shift + Enter para nova linha</div></div></section><aside className="chat-side"><div className="panel context-card"><div className="panel-head"><div><h2>Regras do fluxo</h2><p>O assistente sempre valida</p></div><ShieldCheck className="shield-icon" /></div><ul><li><Check /> OM ou equipamento válido</li><li><Check /> Item da classe de manutenção</li><li><Check /> Saldo oficial PL33 e área</li><li><Check /> Aprovações GDOP / G / GG / D</li></ul></div><div className="panel context-card"><div className="panel-head"><div><h2>Atalhos rápidos</h2><p>Comece por uma opção</p></div></div><button className="quick-action" onClick={() => setDraft('Quero consultar uma OM')}><Search /> Consultar uma OM</button><button className="quick-action" onClick={() => setDraft('Preciso solicitar um material')}><PackageCheck /> Solicitar material</button><button className="quick-action" onClick={() => setDraft('Quero consultar o estoque')}><Boxes /> Consultar estoque</button></div></aside></div> }

...
function FailureAnalysis() { const components = [{ name: 'Placas de Desgaste SAE1045', mtbf: '1.240 h', mttr: '6,2 h', life: 78 }, { name: 'Polias Krupp', mtbf: '980 h', mttr: '8,5 h', life: 42 }, { name: 'Jogo de Vedações', mtbf: '720 h', mttr: '4,1 h', life: 24 }]; return <section className="panel full-panel failure-analysis"><div className="panel-head"><div><h2>Análise de Falhas e Peças de Desgaste</h2><p>MTBF, MTTR e vida útil remanescente · Oficina de Moldes</p></div><StatusBadge tone="blue">Análise preditiva</StatusBadge></div><div className="table-wrap"><table><thead><tr><th>Componente crítico</th><th>MTBF</th><th>MTTR</th><th>Vida útil remanescente</th><th>Recomendação</th></tr></thead><tbody>{components.map((component) => <tr key={component.name}><td><b>{component.name}</b></td><td>{component.mtbf}</td><td>{component.mttr}</td><td><div className="life-cell"><div className="life-track"><i style={{ width: `${component.life}%` }} /></div><b>{component.life}%</b></div></td><td>{component.life < 30 ? <StatusBadge tone="amber">Reordenar antes da falha</StatusBadge> : <StatusBadge tone="green">Dentro do ciclo</StatusBadge>}</td></tr>)}</tbody></table></div></section> }

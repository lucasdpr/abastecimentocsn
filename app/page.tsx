...
import pl33Items from '../data/pl33-items.json'
...
const inspectionData = pl33Items.map((item) => {
  const [month, day, year] = item.data_necessidade.split('/').map(Number)
  const date = year ? `${year < 100 ? 2000 + year : year}-${String(month).padStart(2, '0')}-${String(day).padStart(2, '0')}` : ''
  const quantity = Number(item.quantidade.replace(/,/g, '')) || 0
  const unitPrice = Number(item.preco_medio.replace(/,/g, '')) || 0
  return { id: item.id, equipamento: item.material, status: item.status_item === 'ELIMINADO' ? 'Eliminado' : item.status_item || 'Sem status', data_inspecao: date, responsavel: item.grupo_planejamento || 'PL33', custo_total: quantity * unitPrice, descricao: item.descricao, quantidade: item.quantidade, unidade: item.unidade, ordem: item.ordem, reserva: item.reserva, local: item.local, texto_acao: item.texto_acao }
})

const inspectionTotal = inspectionData.reduce((total, item) => total + item.custo_total, 0)
const inspectionAverage = inspectionData.length ? inspectionTotal / inspectionData.length : 0
const inspectionStatusCounts = inspectionData.reduce<Record<string, number>>((counts, item) => { counts[item.status] = (counts[item.status] || 0) + 1; return counts }, {})
...
function MetricCard({ label, value, detail, icon: Icon, tone, onClick, code = 'FUP-KPI-01', status = 'Liberado' }: { label: string; value: string; detail: string; icon: typeof Activity; tone: string; onClick?: () => void; code?: string; status?: string }) {
  return <button className="metric-card sap-tile" onClick={onClick} title={`Abrir ${label}`}>
  <div className="sap-tile-top"><small>{code}</small><StatusBadge tone={status === 'Crítico' ? 'amber' : status === 'Semanas em atraso' ? 'purple' : 'green'}>{status}</StatusBadge></div><div className={`metric-icon ${tone}`}><Icon /></div>
  <div className="metric-copy"><span>{label}</span><strong>{value}</strong><small>{detail}</small></div><div className="sparkline"><i /><i /><i /><i /><i /><i /></div>
...
export default function Page() {
  ...
  const [notifications, setNotifications] = useState(['Material MOL-SG-004 pronto para retirada na PL33', 'Pedido 4500098214 atualizado pelo fornecedor', 'RC 10004582 aguardando emissão de PO'])
  ...
  const [sapDetails, setSapDetails] = useState<{ title: string; document: string } | null>(null)
  const [sapToast, setSapToast] = useState('')

  useEffect(() => {
    function handleSapDetails(event: Event) { const detail = (event as CustomEvent<{ title: string; document: string }>).detail; setSapDetails(detail) }
    window.addEventListener('oms:sap-details', handleSapDetails)
    return () => window.removeEventListener('oms:sap-details', handleSapDetails)
  }, [])
  ...
  return <div className={`app-shell ${darkMode ? 'dark-mode' : ''}`}>
    ...
    <main className="main-content">
      ...
      {sapDetails && <SapIntegrationModal detail={sapDetails} onClose={() => setSapDetails(null)} />}{paletteOpen && <CommandPalette onClose={() => setPaletteOpen(false)} onNavigate={(nextView) => { setView(nextView); setPaletteOpen(false) }} />}
      {view === 'Assistente OMS' ? <ChatView messages={messages} draft={draft} setDraft={setDraft} sendMessage={sendMessage} /> : <>
        ...
        <div className="page-heading"><div>{view === 'Visão geral' ? <div className="welcome-banner"><div className="welcome-pattern" /><div className="welcome-copy"><p className="eyebrow" suppressHydrationWarning>{isMounted ? 'Central de Abastecimento · PL33' : '\u00a0'}</p><h1>Bom dia, {user?.name || 'Rafael'}.</h1><p className="subtitle">Acompanhe o abastecimento e as demandas da sua oficina.</p></div><div className="welcome-stats"><span><b>14</b> itens ativos</span><span><b>4</b> pendências</span><span><b>98%</b> SLA</span></div></div> : <><p className="eyebrow" suppressHydrationWarning>{isMounted ? 'Quarta-feira, 25 de setembro de 2026' : '\u00a0'}</p><h1>{view}</h1><p className="subtitle">Acompanhe e gerencie o fluxo oficial da Central de Abastecimento.</p></>}</div><button className="primary-button" title="Abrir formulário de nova solicitação" onClick={() => setRequestModal(true)}><MessageSquareText /> Nova solicitação</button></div>{requestModal && <RequestModal onClose={() => setRequestModal(false)} onSaved={() => { setRequestModal(false); setRequestToast('Nova solicitação criada com sucesso!'); setTimeout(() => setRequestToast(''), 2500) }} />}{requestToast && <div className="toast success"><Check />{requestToast}</div>}
...
function SapIntegrationModal({ detail, onClose }: { detail: { title: string; document: string }; onClose: () => void }) { return <div className="modal-backdrop" onClick={onClose}><div className="sap-modal" role="dialog" aria-modal="true" onClick={(event) => event.stopPropagation()}><div className="sap-modal-head"><div><p className="eyebrow">SAP S/4HANA · MM / PM</p><h2>{detail.title}</h2></div><button className="modal-close" onClick={onClose} aria-label="Fechar"><X /></button></div><div className="sap-document"><small>Nº do Documento SAP</small><strong>{detail.document}</strong><StatusBadge tone="green">Documento ativo</StatusBadge></div><div className="sap-grid"><div><small>Centro / Depósito</small><b>1000 / PL33 - Oficina de Moldes</b></div><div><small>Classe de Avaliação</small><b>3000 · Peças de manutenção</b></div><div><small>Grupo de Compradores</small><b>GC-04 · Fundição/Siderurgia</b></div><div><small>Status MM/PM</small><b>ME21N · Pedido criado</b></div></div><div className="sap-history"><div className="sap-history-head"><b>Histórico de Modificações</b><span>Log de transações SAP</span></div><div><span>25/09/2026 · 14:32</span><b>USER_CSN_RAFAEL</b><em>ME21N · Documento criado</em></div><div><span>25/09/2026 · 14:35</span><b>USER_CSN_GPMA</b><em>MIGO · Entrada pendente</em></div><div><span>25/09/2026 · 14:41</span><b>USER_CSN_FISCAL</b><em>MIRO · Fatura liberada</em></div></div><button className="primary-button" onClick={onClose}><Check /> Fechar ficha SAP</button></div></div> }
...
function SupplyFlowWidget() { const [items, setItems] = useState([{ name: 'Resfriador Kawasaki', document: 'PO 4500098214', status: 'Pronto para Retirada', tone: 'green' }, { name: 'Placa Nippon Steel', document: 'PO 4500097988', status: 'Em Trânsito / FUP', tone: 'blue' }, { name: 'Jogo de Vedações Hidráulicas', document: 'RC 10004582', status: 'Aguardando Emissão de PO', tone: 'amber' }]); return <section className="panel supply-flow"><div className="panel-head"><div><h2>Status do Abastecimento & Retirada de Materiais - PL33</h2><p>Oficina de Moldes e Segmentos · Almoxarifado / Depósito PL33</p></div><PackageCheck /></div><div className="supply-list">{items.map((item) => <div className="supply-row" key={item.name}><div className={`supply-status ${item.tone}`}><Check /></div><div className="supply-item"><b>{item.name}</b><small>{item.document} · SAP MM</small></div><StatusBadge tone={item.tone as 'green' | 'blue' | 'amber'}>{item.status}</StatusBadge>{item.status === 'Pronto para Retirada' ? <button className="secondary-button" onClick={() => setItems((current) => current.map((entry) => entry.name === item.name ? { ...entry, status: 'Retirada Confirmada', tone: 'green' } : entry))}><Check /> Confirmar Retirada</button> : item.status === 'Aguardando Emissão de PO' ? <button className="secondary-button" onClick={() => setItems((current) => current.map((entry) => entry.name === item.name ? { ...entry, status: 'PO Emitido · FUP', tone: 'blue' } : entry))}><FileSpreadsheet /> Emitir PO</button> : <button className="secondary-button" onClick={() => setItems((current) => current.map((entry) => entry.name === item.name ? { ...entry, status: 'Baixa Registrada', tone: 'green' } : entry))}><PackageCheck /> Registrar Baixa</button>}</div>)}</div></section> }
...
function Dashboard({ filter, setFilter, filteredRequests, setView }: { filter: string; setFilter: (v: string) => void; filteredRequests: typeof requests; setView: (v: View) => void }) {
  ...
  const [inspectionStatus, setInspectionStatus] = useState('Todos')
  const [inspectionFrom, setInspectionFrom] = useState('')
  const [inspectionTo, setInspectionTo] = useState('')
  const [selectedInspection, setSelectedInspection] = useState<(typeof inspectionData)[number] | null>(null)
  const filteredInspections = inspectionData.filter((item) => {
    const query = inspectionQuery.toLowerCase()
    const matchesQuery = !query || [item.equipamento, item.descricao, item.status, item.responsavel, item.ordem, item.reserva, item.local, String(item.id)].some((value) => value.toLowerCase().includes(query))
    const matchesStatus = inspectionStatus === 'Todos' || item.status === inspectionStatus
    const matchesFrom = !inspectionFrom || item.data_inspecao >= inspectionFrom
    const matchesTo = !inspectionTo || item.data_inspecao <= inspectionTo
    return matchesQuery && matchesStatus && matchesFrom && matchesTo
  })
  function exportInspections() { const headers = ['id', 'equipamento', 'status', 'data_inspecao', 'responsavel', 'custo_total']; const csv = [headers.join(';'), ...filteredInspections.map((item) => headers.map((header) => item[header as keyof typeof item]).join(';'))].join('\\n'); const blob = new Blob([`\\ufeff${csv}`], { type: 'text/csv;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'inspecoes-consolidadas.csv'; link.click(); URL.revokeObjectURL(url) }
  return <div className="content-grid">
    <section className="metrics"><MetricCard label="Custo total" value={currency.format(inspectionTotal)} detail="Soma dos registros consolidados" icon={BarChart3} tone="navy" code="CUSTO-TOTAL" /><MetricCard label="Itens PL33" value={String(inspectionData.length)} detail="Materiais consolidados" icon={ClipboardCheck} tone="amber" code="EQUIP-COUNT" /><MetricCard label="Custo médio" value={currency.format(inspectionAverage)} detail="Por equipamento" icon={Activity} tone="blue" code="CUSTO-MEDIO" /><MetricCard label="Concluídos" value={String(inspectionStatusCounts['Concluído'] || 0)} detail={`${inspectionStatusCounts['Em Manutenção'] || 0} em manutenção`} icon={PackageCheck} tone="green" code="STATUS-OK" /></section>
    <section className="analytics-grid"><div className="panel chart-panel"><div className="panel-head"><div><h2>Desempenho das inspeções</h2><p>Custo total por equipamento · dados consolidados</p></div><span className="chart-period">2026</span></div><div className="stacked-chart"><div className="y-axis"><span>2.000</span><span>1.500</span><span>1.000</span><span>500</span><span>0</span></div><div className="chart-area"><div className="grid-lines"><i /><i /><i /><i /><i /></div><div className="chart-bars">{inspectionData.map((item) => [item.equipamento, String(Math.round(item.custo_total / inspectionTotal * 100)), item.status === 'Concluído' ? '18' : '8', item.status === 'Em Manutenção' ? '24' : '10']).map(([month, rm, rc, po]) => <div className="month-bar" key={month}><button className="stack" title={`${month}: filtrar carteira`} onClick={() => setView('Follow-Up (FUP)')}><i style={{ height: `${rm}%` }} /><i className="bar-rc" style={{ height: `${rc}%` }} /><i className="bar-po" style={{ height: `${po}%` }} /></button><span>{month}</span></div>)}</div></div></div><div className="chart-legend"><span><i className="dot navy" />RM</span><span><i className="dot blue" />RC</span><span><i className="dot yellow" />PO</span></div></div><div className="panel chart-panel status-chart"><div className="panel-head"><div><h2>Distribuição por status</h2><p>Equipamentos inspecionados</p></div></div><div className="status-chart-body"><div className="donut pro-donut"><div><strong>{inspectionData.length}</strong><span>equipamentos</span></div></div><div className="chart-legend vertical"><button title="Filtrar em manutenção" onClick={() => setView('Ordens de Manutenção')}><i className="dot navy" />Em Manutenção <b>{inspectionStatusCounts['Em Manutenção'] || 0}</b></button><button title="Filtrar concluídos" onClick={() => setView('Relatórios')}><i className="dot green-dot" />Concluído <b>{inspectionStatusCounts['Concluído'] || 0}</b></button></div></div></div><div className="panel chart-panel trend-chart"><div className="panel-head"><div><h2>Aprovações e lead time</h2><p>Tendência semanal de atendimento</p></div><span className="trend-value">2,4 dias</span></div><div className="line-chart"><svg viewBox="0 0 500 120" role="img" aria-label="Tendência de aprovações e lead time"><path className="line-fill" d="M0 92 C55 75 70 82 105 65 S170 70 210 48 S275 65 315 35 S390 42 430 24 S470 32 500 10 V120 H0 Z" /><path className="line-path" d="M0 92 C55 75 70 82 105 65 S170 70 210 48 S275 65 315 35 S390 42 430 24 S470 32 500 10" /></svg><div className="line-labels"><span>Sem 1</span><span>Sem 2</span><span>Sem 3</span><span>Sem 4</span><span>Sem 5</span><span>Sem 6</span></div></div></div></section>
    ...
    <SupplyFlowWidget />
    <ImpactSimulator />
    <section className="panel requests-panel"><div className="panel-head"><div><h2>Materiais e reservas consolidados</h2><p>Dados reais da PL33 · ordens, reservas, necessidades e custos</p></div><div className="inspection-head-actions"><span className="approval-progress">{filteredInspections.length} de {inspectionData.length}</span><button className="secondary-button" onClick={exportInspections}><Download /> Exportar listagem</button></div></div><div className="inspection-filters"><label className="inspection-search"><Search /><input value={inspectionQuery} onChange={(event) => setInspectionQuery(event.target.value)} placeholder="Buscar material, ordem, reserva ou grupo PM" /></label><label><span>Status</span><select value={inspectionStatus} onChange={(event) => setInspectionStatus(event.target.value)}><option>Todos</option><option>Em Manutenção</option><option>Concluído</option></select></label><label><span>De</span><input type="date" value={inspectionFrom} onChange={(event) => setInspectionFrom(event.target.value)} /></label><label><span>Até</span><input type="date" value={inspectionTo} onChange={(event) => setInspectionTo(event.target.value)} /></label><button className="text-button" onClick={() => { setInspectionQuery(''); setInspectionStatus('Todos'); setInspectionFrom(''); setInspectionTo('') }}>Limpar</button></div><div className="table-wrap"><table><thead><tr><th>ID</th><th>Material</th><th>Status do item</th><th>Data da necessidade</th><th>Grupo PM</th><th>Valor estimado</th><th>Ações</th></tr></thead><tbody>{filteredInspections.map((item) => <tr key={item.id}><td><b>#{item.id}</b></td><td>{item.equipamento}</td><td><StatusBadge tone={item.status === 'Concluído' ? 'green' : 'amber'}>{item.status}</StatusBadge></td><td>{new Intl.DateTimeFormat('pt-BR').format(new Date(`${item.data_inspecao}T12:00:00`))}</td><td>{item.responsavel}</td><td><b>{currency.format(item.custo_total)}</b></td><td><button className="row-action" title="Abrir relatório detalhado" aria-label={`Abrir relatório de ${item.equipamento}`} onClick={() => setSelectedInspection(item)}><FileText /></button></td></tr>)}{filteredInspections.length === 0 && <tr><td colSpan={7} className="empty-state">Nenhum registro encontrado para os filtros selecionados.</td></tr>}</tbody></table></div>{selectedInspection && <InspectionDetailModal item={selectedInspection} onClose={() => setSelectedInspection(null)} />}</section>
...
function InspectionDetailModal({ item, onClose }: { item: (typeof inspectionData)[number]; onClose: () => void }) { return <div className="modal-backdrop" onClick={onClose}><div className="inspection-detail-modal" role="dialog" aria-modal="true" aria-labelledby="inspection-detail-title" onClick={(event) => event.stopPropagation()}><div className="panel-head"><div><p className="eyebrow">Relatório detalhado</p><h2 id="inspection-detail-title">{item.equipamento}</h2></div><button className="modal-close" onClick={onClose} aria-label="Fechar relatório"><X /></button></div><div className="detail-grid"><div><span>ID</span><b>#{item.id}</b></div><div><span>Status</span><StatusBadge tone={item.status === 'Concluído' ? 'green' : 'amber'}>{item.status}</StatusBadge></div><div><span>Data de inspeção</span><b>{new Intl.DateTimeFormat('pt-BR').format(new Date(`${item.data_inspecao}T12:00:00`))}</b></div><div><span>Responsável</span><b>{item.responsavel}</b></div><div className="detail-total"><span>Custo total</span><strong>{currency.format(item.custo_total)}</strong></div></div><button className="primary-button" onClick={onClose}>Fechar relatório</button></div></div> }
...
function FupView() {
  ...
  return <section className="fup-view">
    ...
    <section className="panel full-panel"><div className="panel-head"><div><h2>Follow-Up de materiais · SAP MM</h2><p>POs, RCs e materiais · Centro 1000 / Depósito PL33 - Oficina de Moldes</p></div><div className="panel-actions"><div className="filter-tabs"><button className={statusFilter === 'Todos' ? 'selected' : ''} onClick={() => { setStatusFilter('Todos'); setPage(1) }}>Todos</button><button className={statusFilter === 'Atraso' ? 'selected' : ''} onClick={() => { setStatusFilter('Atraso'); setPage(1) }}>Em Atraso</button><button className={statusFilter === 'No Prazo' ? 'selected' : ''} onClick={() => { setStatusFilter('No Prazo'); setPage(1) }}>No Prazo</button></div><StatusBadge tone="blue">14 itens acompanhados</StatusBadge><select className="export-select" value={exportMode} onChange={(event) => setExportMode(event.target.value as 'all' | 'overdue')} aria-label="Tipo de exportação"><option value="all">Visão completa · 8.150 itens</option><option value="overdue">Apenas itens em atraso</option></select><button className="secondary-button" title="Baixar os dados filtrados em CSV" onClick={exportCsv}><Download /> Exportar para SAP (.XLSX/CSV)</button><button className="secondary-button sap-sync-button" onClick={() => { setToast('Sincronizando 14 itens com o módulo SAP MM/PM... Sucesso!'); setTimeout(() => setToast(''), 3000) }}><RefreshCw /> Sincronizar com SAP S/4HANA</button></div></div><div className="fup-summary"><div><span>Total</span><b>{currency.format(total)}</b></div><div><span>Com retorno</span><b className="green-text">{currency.format(withReturn)}</b></div><div><span>Sem retorno</span><b className="amber-text">{currency.format(total - withReturn)}</b></div></div><TableTools query={query} setQuery={setQuery} page={page} setPage={setPage} total={filtered.length} /><div className="table-wrap"><table><thead><tr><SortableHeader label="Material" active={false} direction="asc" onSort={() => {}} /><th>Descrição</th><th>Pedido</th><th>Fornecedor</th><th>Prazo</th><th>Retorno</th><SortableHeader label="Valor" active={true} direction={sortAsc ? 'asc' : 'desc'} onSort={() => setSortAsc(!sortAsc)} /></tr></thead><tbody>{visible.map((item) => <tr key={item.po} className="sap-clickable-row" onClick={() => window.dispatchEvent(new CustomEvent('oms:sap-details', { detail: { title: `Pedido ${item.po} · ${item.description}`, document: `PO ${item.po}` } }))}><td><b>{item.material}</b><span>Item {item.item}</span></td><td>{item.description}</td><td>{item.po}</td><td>{item.supplier}</td><td>{item.date}</td><td><StatusBadge tone={item.return === 'Sim' ? 'green' : 'amber'}>{item.status}</StatusBadge></td><td><b>{currency.format(item.value)}</b></td></tr>)}</tbody></table></div>{toast && <div className="toast success"><Check />{toast}</div>}</section>
  </section>
}

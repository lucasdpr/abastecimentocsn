...
const inspectionData = [
  { id: 1, equipamento: 'Segmento 01', status: 'Em Manutenção', data_inspecao: '2026-09-20', responsavel: 'João Silva', custo_total: 1250.00 },
  { id: 2, equipamento: 'Molde B', status: 'Concluído', data_inspecao: '2026-09-22', responsavel: 'Maria Santos', custo_total: 850.50 },
]

const inspectionTotal = inspectionData.reduce((total, item) => total + item.custo_total, 0)
const inspectionAverage = inspectionTotal / inspectionData.length
...
function Dashboard({ filter, setFilter, filteredRequests, setView }: { filter: string; setFilter: (v: string) => void; filteredRequests: typeof requests; setView: (v: View) => void }) {
  ...
  const filteredInspections = inspectionData.filter((item) => {
    ...
    const matchesQuery = !query || [item.equipamento, item.status, item.responsavel, String(item.id)].some((value) => value.toLowerCase().includes(query))

...
function FupView() {
  const [query, setQuery] = useState(''); const [page, setPage] = useState(1); const [sortAsc, setSortAsc] = useState(false); const [toast, setToast] = useState('');
  ...
  function exportCsv() { const csv = ['Material,Descrição,Pedido,Fornecedor,Prazo,Status,Valor', ...filtered.map((item) => [item.material, item.description, item.po, item.supplier, item.date, item.status, item.value].join(','))].join('\\n'); const link = document.createElement('a'); link.href = URL.createObjectURL(new Blob([csv], { type: 'text/csv;charset=utf-8' })); link.download = 'oms-fup.csv'; link.click(); setToast('Relatório baixado em CSV!'); setTimeout(() => setToast(''), 2200) }
